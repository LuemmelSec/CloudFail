...
def phase_init(target: str, cf_ranges: List[str]) -> ScanResult:
    """Resolve target domain and check if it is behind Cloudflare."""
...
def phase_passive(
    ...
    """Collect candidate IPs and subdomains using purely passive sources."""
...
def phase_subdomain(
    ...
    """Resolve subdomains from CT names + wordlist; flag non-CF IPs."""
    ...
    try:
        ...
        logger.warning(f"Wordlist not found: {wl_path}. Using CT names only.")
    ...
    if dns_history.check_wildcard(result.target):
        logger.warning(
            "Wildcard DNS detected — all subdomains resolve. "
            "Bruteforce results unreliable; CT + passive names are still valid."
        )
    ...
    # Sort: non-CF first (most interesting), then CF
    hits.sort(key=lambda h: h["behind_cloudflare"])
    result.subdomain_hits = hits
    non_cf_count = sum(1 for h in hits if not h["behind_cloudflare"])
    logger.info(
        f"Resolved {len(hits)} hostnames — "
        f"[bold green]{non_cf_count} not behind Cloudflare[/bold green]."
    )


def phase_filter(result: ScanResult, cf_ranges: List[str]) -> None:
    """Enrich all candidate IPs with ASN data; surface non-CF ones."""
    ...
    logger.info(
        ...
        f"(CF-range IPs skip ASN lookup to conserve quota)…"
    )
    ...
    if result.non_cf_ips:
        logger.success(
            f"[bold]{len(result.non_cf_ips)} non-Cloudflare IP(s)[/bold] discovered!"
        )
    else:
...
def render_results(
    ...
    """Print results to console; optionally write to file."""
    ...
    # Subdomain table — capped to avoid terminal flooding
    if result.subdomain_hits:
        ...
        if total > cap:
            logger.info(
                f"{total - cap} additional subdomain hits not shown. "
                f"Use --output json --output-file results.json to see all."
            )
...
def main() -> None:
    ...
    # 1. SSL verification — must happen before ANY http call
    ...
    if not cf_ranges:
        logger.error(
            ...
            "  • Behind a corporate proxy?  Add --no-verify-ssl\n"
            "  • No internet connectivity?  The built-in fallback should have kicked in.\n"
            "  • Try: python -m cloudfail ... --update-ranges --no-verify-ssl"
        )
        sys.exit(1)
    ...
    if args.passive_only:
        logger.info("--passive-only: skipping subdomain bruteforce phase.")
    else:
